...
android-signing-key: docker-android-base
	if [ ! -e $(KEYSTORE) ]; then \
		$(docker) $(docker_run_cmd) --workdir /go/katzen katzen/android_sdk bash -c "keytool -genkey -keystore $(KEYSTORE) -storepass ${KEYPASS} -alias android -keyalg RSA -keysize 2048 -validity 10000 -noprompt -dname CN=android"; \
	fi
...
docker-go-mod-go-get: docker-debian-base
	$(docker) $(docker_run_cmd) --rm katzen/debian_base \
			bash -c 'cd /go/katzen; go get'

docker-go-mod-update: docker-debian-base
	$(docker) $(docker_run_cmd) -v "$(shell readlink -f .)":/go/katzen --rm katzen/debian_base \
...
docker-shell: docker-debian-base
	$(docker) $(docker_run_cmd) --rm -it katzen/debian_base bash
